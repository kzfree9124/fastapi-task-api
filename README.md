# タスク管理API (FastAPI + JWT + SQLAlchemy)
...
JWT_ALGORITHM=HS256
```
- `DATABASE_URL` は PostgreSQL の接続情報です  
  （docker-compose の設定に合わせて `<USER>`, `<PASSWORD>`, `<HOST>`, `<DB_NAME>` を変更してください）
- `JWT_SECRET` は JWT の署名に使用する秘密鍵です  
  （十分に長いランダムな文字列を設定してください）
- `JWT_ALGORITHM` は署名アルゴリズムです
